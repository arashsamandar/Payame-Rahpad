...
class AjaxController extends Controller
{
        ...
        if($request->ajax()) {
            if(USC::CheckPermission($request->id)) {
                ...
                Logs::create([
                    ...
                    'user_id' => $contact->id,
                    'logCode' => '011',
                    'log_desc' => 'account updated successfully',
                    'Reserved1' => $userid,
                    'Reserved2' => $contact->id,
                ]);
    ...
    public function changepass(Request $request)
    {
        ...
        if (USC::CheckPermission($myuserid)) {
            ...
            Logs::create([
                ...
                'user_id' => $myuserid,
                'logCode' => '013',
                'log_desc' => 'password changed successfully',
                'Reserved1' => $userid,
                'Reserved2' => $myuserid,
            ]);
    ...
    public function show_access(Request $request) {
        if($request->ajax()) {
            ...
            if(!empty($user->permissions()->get())) {
                $ar = json_decode($user, true);
                if (!empty($user->permissions()->first()->access_users)) {
                    $per_array['acuser'] = $user->permissions()->first()->access_users;
                } else {
                    $per_array['acuser'] = 0;
                }
                if (!empty($user->permissions()->first()->access_contents)) {
                    $per_array['accont'] = $user->permissions()->first()->access_contents;
                } else {
                    $per_array['accont'] = 0;
                }
                array_push($ar, $per_array);
                return response($ar);
            }
            } else {
                return response('fail');
    ...
    public function change_access(Request $request) {
        if($request->ajax()) {
            if(\Auth::guard('admin')->check()) {
                ...
                if (!empty($user->permissions()->first()->id)) {
                    $permit = Permissions::find($user->permissions()->first()->id);
